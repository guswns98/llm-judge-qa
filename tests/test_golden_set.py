...
GOLDEN_SET = [
    # --- 5점: 완벽한 답변 ---
    {
        ...
        "response": "PER(Price to Earnings Ratio, 주가수익비율)은 주가를 주당순이익(EPS)으로 나눈 값입니다. "
                     ...
                     "동일 업종 내 비교에 활용됩니다.",
        "expected_relevance": 5, "expected_accuracy": 5, "expected_completeness": 5,
    ...
    },
    {
        ...
        "response": "ETF는 증권사 앱에서 주식처럼 매수할 수 있습니다. 코스피200 ETF, 나스닥100 ETF 등 다양한 상품이 있으며, "
                     "소액으로도 분산투자가 가능합니다.",
        "expected_relevance": 5, "expected_accuracy": 5, "expected_completeness": 3,
        "reason": "기본 설명은 맞지만 수수료, 추적오차 등 투자 시 고려사항 부족",
    },

    # --- 3점대: 관련성은 있으나 정확성 또는 완결성 중 하나가 미흡 ---
    {
        "question": "MACD 사용법",
        "response": "MACD는 주가의 추세를 파악하는 기술적 지표입니다. 매수/매도 신호를 확인할 수 있습니다.",
        "expected_relevance": 4, "expected_accuracy": 3, "expected_completeness": 2,
        "reason": "관련은 있으나 MACD선/시그널선 교차, 히스토그램 등 핵심 사용법 설명 없음",
    },
    {
        "question": "금리 인상이 주식시장에 미치는 영향",
        "response": "금리가 오르면 주식시장에 부정적인 영향을 줄 수 있습니다.",
        "expected_relevance": 5, "expected_accuracy": 4, "expected_completeness": 2,
        "reason": "방향은 맞지만, 왜 부정적인지(할인율, 채권 매력도 등) 구체적 설명 없음",
    },

    # --- 2점대: 관련은 있으나 답변이 불충분하거나 일부 오류 ---
    {
        "question": "볼린저밴드 설명",
        "response": "볼린저밴드는 주가 차트에서 사용하는 선입니다.",
        "expected_relevance": 4, "expected_accuracy": 3, "expected_completeness": 1,
        "reason": "관련은 있으나 상한선/중심선/하한선, 표준편차 기반 등 핵심 내용 전무",
    },
    {
        "question": "배당수익률 높은 종목 추천",
        "response": "배당수익률이 높은 종목으로는 한국전력이 있습니다. 배당수익률은 약 15%입니다.",
        "expected_relevance": 3, "expected_accuracy": 1, "expected_completeness": 2,
        "reason": "한국전력은 적자로 배당이 불안정하며, 15% 수치는 부정확 (할루시네이션)",
    },
    ...
    # --- 1점: 질문과 무관하거나 에러 ---
    {
        ...
        "expected_relevance": 1, "expected_accuracy": 1, "expected_completeness": 1,
        "reason": "에러 메시지만 반환",
    },
    {
...
class TestGoldenSet:
    ...
    def test_evaluator_reliability(self):
        """Golden Set으로 LLM 평가자의 3축 채점 신뢰도를 검증"""
        ...
        for i, item in enumerate(GOLDEN_SET, start=1):
            ...
            expected = {axis: item[f"expected_{axis}"] for axis in AXIS_LABELS}
            print(f"  기대: 관련성={expected['relevance']} 정확성={expected['accuracy']} 완결성={expected['completeness']} ({item['reason']})")

            eval_result = evaluate_with_ollama(
                item["question"], item["response"], index=i
            )

            # 3축 각각 ±TOLERANCE 이내인지 판정
            axis_diffs = {}
            axis_matches = {}
            for axis in AXIS_LABELS:
                actual = eval_result.get(axis, 0)
                exp = expected[axis]
                axis_diffs[axis] = abs(actual - exp)
                axis_matches[axis] = axis_diffs[axis] <= TOLERANCE

            is_match = all(axis_matches.values())
            if is_match:
                matched += 1

            status = "✓ PASS" if is_match else "✗ FAIL"
            axis_detail = " | ".join(
                f"{AXIS_LABELS[a]}:{eval_result.get(a, 0)}(기대{expected[a]},차이{axis_diffs[a]})"
                for a in AXIS_LABELS
            )
            print(f"  LLM: {axis_detail} | {status}")
            ...
            results.append({
                ...
                "expected": expected,
                "actual": {axis: eval_result.get(axis, 0) for axis in AXIS_LABELS},
                "axis_diffs": axis_diffs,
                "is_match": is_match,
                "reason": item["reason"],
        ...
        print(f"\n{'='*50}")
        print(f"Golden Set 검증 결과 (3축 독립 비교)")
        print(f"일치율: {matched}/{total} ({match_rate:.1%})")
        print(f"허용 오차: 축별 ±{TOLERANCE}점 (3축 모두 충족 시 일치)")
